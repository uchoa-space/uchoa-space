...
- [ ] The landing's description is read as it renders, not as it was written. The copy decision this criterion anticipated has been taken, and it was taken for Slack: links to this site get forwarded inside a company, so where clients conflict Slack wins. The lede is now `og:title`, and the description is 144 characters that render whole on Slack and Discord. X and WhatsApp cut it — X after "agent runtimes", WhatsApp mid-"uptime" — and that is an accepted trade, not a defect to report. What is left to check on a real client is that Slack really does render both in full, and that the title is not itself truncated anywhere
